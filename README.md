# iOS 5GPN WhatsApp-补丁 



给"只设 DNS 就能接入内网"的网关(**5GPN / privdns-gateway** )的一键补丁。这类网关脚本可能适配在安卓手机上运转正常，但iOS上的 WhatsApp **消息发不出去**——本补丁可一键修复这个问题,且不改变你使用的方式。

## 一键安装

```bash
# 1) 先干跑测测试:只检测你的网关、不做任何改动(推荐先跑这个):
...
这些项目的原理都一样:你手机的 **DoT DNS** 把"要代理"的域名解析成**网关自己的 IP**,网关再按 **TCP/443 上明文的 TLS SNI** 来转发(用 sniproxy、sing-box 或 HAProxy)。凡是带 SNI 的流量都能覆盖—。

但 **WhatsApp iOS版的聊天链接根本没有 SNI**。它是直接使用特有的 TCP/443 上的 Noise 协议握手,没有 TLS 层,所以 SNI 路由器读不到任何主机名。它的首字节是:

```
45 44 00 01   "ED…"   (多设备 edge)
57 41 06 03   "WA…"   (WhatsApp标识)
```

网关看到一个无法分类的 443 连接,只能把它丢掉 → 你的消息一直转圈、发送失败。
